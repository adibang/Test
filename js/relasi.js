...
async function checkSession() {
    try {
        ...
        if (typeof dbGet !== 'function') {
            throw new Error('dbGet tidak ditemukan. Pastikan database.js dimuat.');
        }
        ...
        if (nameEl) nameEl.textContent = user ? user.name : parsed.name;
        return true;
    } catch (e) {
        console.error('Error checkSession:', e);
        showDebugError('checkSession gagal: ' + e.message);
        window.location.href = 'index.html';
        return false;
    }
}
...
async function initRelasiPage() {
    ...
    try {
        ...
        if (typeof initDatabase !== 'function') throw new Error('initDatabase tidak ditemukan');
        await initDatabase();

        const sessionOk = await checkSession();
        if (!sessionOk) return;

        if (typeof loadCustomers !== 'function') throw new Error('loadCustomers tidak ditemukan');
        await loadCustomers();

        if (typeof loadSuppliers !== 'function') throw new Error('loadSuppliers tidak ditemukan');
        await loadSuppliers();
...
function showDefaultPage() {
    ...
    else showDebugError('Elemen #relasi-content tidak ditemukan');
}
...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (typeof showLoading !== 'function') throw new Error('showLoading tidak ditemukan. Pastikan ui.js dimuat.');
        if (typeof hideLoading !== 'function') throw new Error('hideLoading tidak ditemukan. Pastikan ui.js dimuat.');
        if (typeof showNotification !== 'function') throw new Error('showNotification tidak ditemukan. Pastikan ui.js dimuat.');
        ...
        if (typeof dbGet !== 'function') throw new Error('dbGet tidak ditemukan. Pastikan database.js dimuat.');
        if (typeof dbPut !== 'function') throw new Error('dbPut tidak ditemukan. Pastikan database.js dimuat.');
        if (typeof dbAdd !== 'function') throw new Error('dbAdd tidak ditemukan. Pastikan database.js dimuat.');
        if (typeof dbDelete !== 'function') throw new Error('dbDelete tidak ditemukan. Pastikan database.js dimuat.');
